  ...
  return (
    <div className="flex min-h-screen flex-col bg-black">
      ...
      <main className="flex-1 p-6 max-w-5xl mx-auto w-full space-y-8 page-enter">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {games.map((game) => (
            <Link key={game.slug} href={`/games/${game.slug}`} className="group">
              <div className="border border-white/5 bg-white/[0.02] hover:border-neon-green/30 transition-all duration-300 h-full flex flex-col">
                {/* Header */}
                ...
                <div className="p-5 flex-1 space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-3 pt-2">
                    <div>
                      <div className="text-[10px] font-mono text-gray-700 uppercase">House_Edge</div>
                      <div className="text-sm font-mono text-white font-bold">{game.stats.edge}</div>
                    </div>
                    <div>
                      <div className="text-[10px] font-mono text-gray-700 uppercase">Max_Win</div>
                      <div className="text-sm font-mono text-neon-green font-bold">{game.stats.maxWin}</div>
                    </div>
                    <div>
                      <div className="text-[10px] font-mono text-gray-700 uppercase">Finality</div>
                ...
                <div className="border-t border-white/5 px-5 py-3 flex items-center justify-between">
                  <span className="text-xs font-mono text-gray-600 group-hover:text-neon-green transition-colors uppercase">
                    Initialize_Terminal
                  </span>
                  <ArrowRight className="w-4 h-4 text-gray-700 group-hover:text-neon-green group-hover:translate-x-1 transition-all" />
                </div>
              </div>
            </Link>
          ))}
        ...
        {/* Protocol Info */}
        <div className="border border-white/5 bg-white/[0.02] p-5 flex items-center gap-4">
      ...
      <footer className="border-t border-white/5 px-4 py-3 flex justify-between items-center">
        <div className="text-[10px] font-mono text-gray-700">ENTROPY_PROTOCOL // MONAD_TESTNET</div>
        <div className="text-[10px] font-mono text-gray-700">2 TERMINALS ACTIVE</div>
      </footer>
    </div>
  );
}
